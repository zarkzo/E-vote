...
function printHasil($data) {
  ...
  $foto_waketum = $path . $data["foto_waketum"];
  $no_paslon = $data["no_paslon"];
  $jumlah_vote = $data["jumlah_vote"];  // Menambahkan jumlah suara
  $data_form = "hasilVote-" . $data["no_paslon"];
  
  // Template dengan penambahan jumlah suara
  $template = <<<EOT
    <div class="row bg-white rounded shadow-sm mx-auto px-3 py-3 mt-3 border border-5 border-top-0 border-end-0 border-bottom-0 border-success">
      <span class="text-center fw-bold text-secondary fs-1 mb-3 bg-light">
        $no_paslon
      ...
      <!-- Menampilkan jumlah suara -->
      <div class="col-12 mt-3 text-center">
        <span class="text-success fw-bold fs-4">Jumlah Suara: $jumlah_vote</span>
      </div>
    </div>
EOT;
